...
- 倒计时支持时、分、秒输入、暂停/继续，以及快捷时长按钮；右键快捷按钮可修改其分钟数；到 0 后可让悬浮窗红色闪动。
- 支持鼠标穿透、REC 指示器、光环边框、分段计时和全局快捷键。
- 常驻 Windows 系统托盘；关闭或最小化控制器时会隐藏到右下角图标，可从托盘恢复或退出。
- 支持 English / 中文 界面切换。
...
7. 双击右下角托盘图标可恢复控制器；右键图标可退出程序。
8. 倒计时运行时，双击悬浮计时器可恢复主控制器；可在选项中启用“倒计时结束时闪动悬浮窗”。
